...
import { BookOpen, Edit, Trash2, Link as LinkIcon, UploadCloud, Loader } from 'lucide-react';
...
const ClassLogTab = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (file.size > 10485760) {
      alert("This file is too large (over 10MB). Please upload it to Google Drive and use the 'Paste Link' option instead.");
    ...
    try {
      ...
      // Save the secure Cloudinary URL to our form data
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="bg-purple-50 p-4 md:p-6 rounded-xl border border-purple-100 mb-8">
        ...
        <div className="mb-6"><label className="text-xs font-bold text-gray-600 uppercase">Homework Assigned</label><textarea rows="2" value={formData.homework} onChange={e => setFormData({...formData, homework: e.target.value})} className="w-full p-2 border rounded mt-1 text-sm resize-y" placeholder="Leave blank if none..." /></div>
        
        {/* --- SMART ATTACHMENT SECTION --- */}
        <div className="mb-6 bg-white p-4 rounded-lg border border-purple-100 shadow-sm">
          <label className="text-xs font-bold text-gray-600 uppercase mb-3 block">Study Material / Attachment</label>
          
          {/* Toggle Buttons */}
          <div className="flex flex-wrap gap-2 mb-4">
            <button type="button" onClick={() => {setAttachMode('none'); setFormData({...formData, attachmentLink: ''})}} className={`px-4 py-2 rounded text-sm font-bold border ${attachMode === 'none' ? 'bg-purple-100 border-purple-300 text-purple-700' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'}`}>No Attachment</button>
            <button type="button" onClick={() => setAttachMode('upload')} className={`flex items-center gap-2 px-4 py-2 rounded text-sm font-bold border ${attachMode === 'upload' ? 'bg-purple-100 border-purple-300 text-purple-700' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'}`}><UploadCloud size={16}/> Upload File (&lt; 10MB)</button>
            <button type="button" onClick={() => setAttachMode('link')} className={`flex items-center gap-2 px-4 py-2 rounded text-sm font-bold border ${attachMode === 'link' ? 'bg-purple-100 border-purple-300 text-purple-700' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'}`}><LinkIcon size={16}/> Paste Link (Drive/Heavy Files)</button>
          </div>

          {/* Conditional Inputs */}
          {attachMode === 'upload' && (
            <div className="animate-fade-in">
              <input type="file" onChange={handleFileUpload} accept=".pdf,.doc,.docx,.jpg,.png" className="w-full p-2 border rounded text-sm mb-2" disabled={isUploading} />
              {isUploading && <p className="text-sm text-purple-600 font-bold flex items-center gap-2"><Loader size={14} className="animate-spin" /> Uploading to secure server...</p>}
              {!isUploading && formData.attachmentLink && <p className="text-sm text-green-600 font-bold flex items-center gap-1">✓ File ready to be saved.</p>}
            </div>
          )}

          {attachMode === 'link' && (
            <div className="animate-fade-in">
              <input type="url" placeholder="https://drive.google.com/..." value={formData.attachmentLink} onChange={e => setFormData({...formData, attachmentLink: e.target.value})} className="w-full p-2 border rounded text-sm outline-none focus:border-purple-400" />
              <p className="text-xs text-gray-400 mt-1">Paste a Google Drive or external link for large videos/documents.</p>
            </div>
          )}
        </div>

        <div className="flex flex-col md:flex-row gap-2">
          <button type="submit" disabled={isUploading} className="w-full md:w-auto bg-purple-600 text-white px-6 py-2 rounded font-bold hover:bg-purple-700 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
            {editingId ? 'Update Log' : 'Save Log'}
          </button>
          {editingId && <button type="button" onClick={resetForm} className="w-full md:w-auto bg-gray-300 px-4 py-2 rounded font-bold text-gray-800">Cancel</button>}
        </div>
      </form>

      {/* --- DISPLAY LOGS --- */}
      <div className="space-y-4">
        {logs.length === 0 ? <p className="text-center text-gray-500 italic p-8 bg-gray-50 rounded border border-dashed">No class logs recorded yet.</p> : 
          logs.map(log => (
