...
const exportExamToCSV = async (exam) => {
  ...
  return (
    // ✅ Main Wrapper with overflow-x-hidden
    <div className="w-full bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 animate-fade-in max-w-6xl mx-auto overflow-x-hidden">
      
      {/* ✅ Responsive Header: Stacks items vertically on mobile, side-by-side on larger screens */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b border-gray-100 pb-4 gap-4">
        ...
        {/* Buttons wrap to next line if screen is too small */}
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          {currentUser?.role === 'admin' && viewMode === 'list' && (
            <>
              <button onClick={handleMasterExportExams} className="flex-1 sm:flex-none bg-green-50 text-green-700 border border-green-200 px-3 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-1 hover:bg-green-100 shadow-sm">
                <Download size={16} /> Export All
              </button>
              <button onClick={handleMasterDeleteAllExams} className="flex-1 sm:flex-none bg-red-50 text-red-700 border border-red-200 px-3 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-1 hover:bg-red-100 shadow-sm">
          ...
          {viewMode === 'list' ? (
            <button onClick={() => setViewMode('create')} className="flex-1 sm:flex-none bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 font-bold hover:bg-blue-700 shadow-sm text-sm">
              <Plus size={16} /> New Exam
            </button>
          ) : (
            <button onClick={() => setViewMode('list')} className="flex-1 sm:flex-none bg-gray-200 text-gray-800 px-4 py-2 rounded-lg font-bold hover:bg-gray-300 shadow-sm text-sm">
              Back to Exams
            </button>
          )}
        </div>
      ...
      {viewMode === 'list' && (
        <>
          <div className="mb-4">
            <select value={filterStd} onChange={e => setFilterStd(e.target.value)} className="w-full sm:w-auto p-2 border rounded-lg outline-none bg-gray-50 font-bold text-sm">
              <option value="All">Filter: All Standards</option>
              {STANDARD_OPTIONS.map(std => <option key={std} value={std}>{std}</option>)}
            </select>
          </div>
            ...
            {filteredExams.length === 0 ? <p className="text-gray-500">No exams found.</p> : filteredExams.map(exam => (
              <div key={exam._id} className="border border-gray-200 rounded-xl p-5 hover:shadow-md transition-shadow relative">
                <div className="flex justify-between items-start mb-2 gap-2">
                  <h3 className="font-bold text-lg text-gray-900 break-words">{exam.name}</h3>
                  <span className="bg-blue-100 text-blue-800 text-xs font-bold px-2 py-1 rounded shrink-0">{exam.std}</span>
                </div>
                <p className="text-sm text-gray-600 mb-4">
                  Max Marks: {exam.maxMarks} • Pass: {exam.minPassMarks || 35}<br/>
                  <span className="text-xs">{new Date(exam.examDate).toLocaleDateString()}</span>
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100">
                  <button onClick={() => openGradeMode(exam)} className="flex-1 bg-green-50 text-green-700 p-2 rounded text-xs font-bold flex justify-center items-center gap-1 hover:bg-green-100"><Edit3 size={14}/> Edit Marks</button>
                  <button onClick={() => { setSelectedExam(exam); setViewMode('viewResults'); }} className="flex-1 bg-blue-50 text-blue-700 p-2 rounded text-xs font-bold flex justify-center items-center gap-1 hover:bg-blue-100"><TrendingUp size={14}/> View Results</button>
                  <button onClick={() => exportExamToCSV(exam)} className="p-2 text-gray-500 hover:text-gray-900 bg-gray-50 rounded" title="Export CSV"><Download size={16}/></button>
                  <button onClick={() => handleDeleteExam(exam)} className="p-2 text-red-500 hover:text-red-700 bg-red-50 rounded" title="Delete Exam"><Trash2 size={16}/></button>
                </div>
              </div>
      ...
      {viewMode === 'create' && (
        <form onSubmit={handleCreateExam} className="max-w-2xl w-full bg-gray-50 p-4 md:p-6 rounded-xl border border-gray-200">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div className="sm:col-span-2">
              <label className="font-bold text-sm text-gray-700">Exam Name</label>
              <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-2 border rounded mt-1 text-sm" placeholder="e.g., Mid-Term Mathematics" />
            </div>
            
            <div>
              <label className="font-bold text-sm text-gray-700">Standard</label>
              <select required value={formData.std} onChange={e => setFormData({...formData, std: e.target.value})} className="w-full p-2 border rounded mt-1 bg-white text-sm">
                {STANDARD_OPTIONS.map(std => <option key={std} value={std}>{std}</option>)}
              </select>
            </div>
            
            <div>
              <label className="font-bold text-sm text-gray-700">Batch</label>
              <select value={formData.batch} onChange={e => setFormData({...formData, batch: e.target.value})} className="w-full p-2 border rounded mt-1 bg-white text-sm">
                <option value="All">All Batches</option><option value="Morning">Morning</option><option value="Evening">Evening</option>
              </select>
            </div>

            <div>
              <label className="font-bold text-sm text-gray-700">Max Marks</label>
              <input required type="number" min="1" value={formData.maxMarks} onChange={e => setFormData({...formData, maxMarks: e.target.value})} className="w-full p-2 border rounded mt-1 text-sm" />
            </div>
            
            <div>
              <label className="font-bold text-sm text-gray-700">Min Passing Marks</label>
              <input required type="number" min="0" value={formData.minPassMarks} onChange={e => setFormData({...formData, minPassMarks: e.target.value})} className="w-full p-2 border rounded mt-1 text-sm" />
            </div>
            
            <div className="sm:col-span-2">
              <label className="font-bold text-sm text-gray-700">Exam Date</label>
              <input required type="date" value={formData.examDate} onChange={e => setFormData({...formData, examDate: e.target.value})} className="w-full p-2 border rounded mt-1 text-sm" />
            </div>
          </div>
          <button type="submit" className="w-full sm:w-auto bg-blue-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-blue-700 text-sm">Save Exam & Continue</button>
        </form>
      )}
      ...
      {viewMode === 'grade' && (
        <div className="max-w-3xl w-full">
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead>
                <tr className="bg-gray-100 border-b">
                  <th className="p-3 text-sm text-gray-700">Student Name</th>
                  <th className="p-3 text-sm text-gray-700">Obtained Marks (out of {selectedExam.maxMarks})</th>
                  <th className="p-3 text-sm text-gray-700 text-center">Absent?</th>
                </tr>
              </thead>
              <tbody>
                {students.filter(s => s.std === selectedExam.std && (selectedExam.batch === 'All' || s.batch === selectedExam.batch)).map(student => (
                  <tr key={student._id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-3">
                      <input 
                        ...
                        value={marksInput[student._id]?.obtainedMarks !== undefined ? marksInput[student._id]?.obtainedMarks : ''} 
                        onChange={(e) => handleMarksChange(student._id, e.target.value, selectedExam.maxMarks)}
                        className="p-2 border rounded w-full sm:w-32 outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-200 text-sm" 
                      />
                    </td>
          ...
          <button onClick={handleSaveMarks} className="w-full sm:w-auto bg-green-600 text-white px-8 py-3 rounded-lg font-bold hover:bg-green-700 shadow-md text-sm">Publish Marks to Database</button>
        </div>
      )}

      {viewMode === 'viewResults' && (
        <div className="w-full">
          {/* ✅ Responsive Header for Results */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 bg-gray-50 p-4 rounded-lg border gap-4">
            <div>
              <p className="text-gray-500 text-sm font-bold">Total Appeared: {selectedExam.marks.length}</p>
              <p className="text-gray-500 text-sm font-bold">Max Marks: {selectedExam.maxMarks} <span className="mx-1 sm:mx-2">|</span> Pass: {selectedExam.minPassMarks || 35}</p>
            </div>
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              <button onClick={() => setSortOrder(sortOrder === 'desc' ? 'asc' : 'desc')} className="flex-1 sm:flex-none bg-white border p-2 rounded text-sm font-bold shadow-sm hover:bg-gray-50">
                Sort: {sortOrder === 'desc' ? 'Highest' : 'Lowest'}
              </button>
              <button onClick={() => exportExamToCSV(selectedExam)} className="flex-1 sm:flex-none bg-green-600 text-white px-4 py-2 rounded text-sm font-bold flex justify-center items-center gap-2 hover:bg-green-700">
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="bg-gray-100 border-b">
                  ...
                  <th className="p-3 text-sm text-gray-700">Student</th>
                  <th className="p-3 text-sm text-gray-700">Marks</th>
                  <th className="p-3 text-sm text-gray-700">Percentage</th>
                  <th className="p-3 text-sm text-gray-700">Status</th>
                </tr>
              </thead>
              <tbody>
                {selectedExam.marks.length === 0 ? <tr><td colSpan="5" className="p-6 text-center text-gray-500 text-sm">No marks entered yet.</td></tr> : 
                  [...selectedExam.marks].sort((a, b) => sortOrder === 'desc' ? b.obtainedMarks - a.obtainedMarks : a.obtainedMarks - b.obtainedMarks)
                  .map((mark, index) => {
                    const percentage = Math.round((mark.obtainedMarks / selectedExam.maxMarks) * 100);
                    const passThreshold = selectedExam.minPassMarks !== undefined ? selectedExam.minPassMarks : 35;
                    ...
                    return (
                      <tr key={index} className="border-b hover:bg-gray-50">
                        <td className="p-3 font-bold text-gray-400 text-sm">#{index + 1}</td>
                        <td className="p-3 font-bold text-gray-900 text-sm">{mark.studentId?.name || 'Unknown'}</td>
                        <td className="p-3 font-bold text-sm">{mark.isAbsent ? 'AB' : mark.obtainedMarks}</td>
                        <td className="p-3 text-sm">{mark.isAbsent ? '0%' : `${percentage}%`}</td>
                        <td className="p-3">
                          {mark.isAbsent ? <span className="text-red-500 flex items-center gap-1 text-xs font-bold"><XCircle size={14}/> Absent</span> :
                           isPass ? <span className="text-green-600 flex items-center gap-1 text-xs font-bold"><CheckCircle size={14}/> Pass</span> : 
                           <span className="text-red-500 flex items-center gap-1 text-xs font-bold"><XCircle size={14}/> Fail</span>}
                        </td>
                      </tr>
                    )
                  })
