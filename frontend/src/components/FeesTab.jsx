...
const FeesTab = () => {
  ...
  return (
    ...
    <div className="w-full bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 animate-fade-in overflow-x-hidden">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 border-b border-gray-100 pb-4 gap-4">
        <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
          <IndianRupee className="text-green-600" /> Accounts & Fees
        </h2>
        
        <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
          {/* Action Tabs - Wrapped for mobile safety */}
          <div className="flex flex-wrap sm:flex-nowrap gap-2 w-full sm:w-auto bg-gray-100 p-1 rounded-lg">
            <button
              onClick={() => setViewMode("collect")}
              className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-md ${viewMode === "collect" ? "bg-white shadow-sm text-green-700" : "text-gray-500 hover:text-gray-800"}`}
            >
              Collect
            </button>
            <button
              onClick={() => setViewMode("history")}
              className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-md ${viewMode === "history" ? "bg-white shadow-sm text-green-700" : "text-gray-500 hover:text-gray-800"}`}
            >
              Ledger
            </button>
            <button
              onClick={() => setViewMode("structure")}
              className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-md ${viewMode === "structure" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-800"}`}
            >
              <Settings size={16} className="inline mr-1" /> Setup
            </button>
          </div>

          {/* Admin Export/Wipe Buttons */}
          {user?.role === "admin" && viewMode === "history" && (
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              <button
                onClick={handleMasterExportPayments}
                className="flex-1 sm:flex-none bg-green-100 text-green-700 px-3 py-2 sm:py-1 rounded text-sm font-bold flex justify-center items-center gap-1"
              >
                <Download size={14} /> Export
              </button>
              <button
                onClick={handleMasterDeleteAllPayments}
                className="flex-1 sm:flex-none bg-red-100 text-red-700 px-3 py-2 sm:py-1 rounded text-sm font-bold flex justify-center items-center gap-1"
              >
      ...
      {viewMode === "collect" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <form
            onSubmit={handleCollectPayment}
            className="w-full bg-green-50/50 border border-green-100 p-4 sm:p-6 rounded-xl relative"
          >
            <h3 className="font-bold text-green-900 mb-4 flex items-center gap-2">
              <PlusCircle size={18} />{" "}
              {editingId ? "Edit Payment" : "New Receipt"}
            ...
            {editingId && (
              <button
                ...
                className="absolute top-4 sm:top-6 right-4 sm:right-6 text-sm text-red-600 font-bold bg-white px-2 py-1 rounded shadow-sm sm:bg-transparent sm:shadow-none"
              >
                Cancel Edit
              </button>
            )}
            ...
            {/* ✅ Responsive Filters */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 mt-8 sm:mt-0">
              <div>
                <label className="text-xs font-bold text-gray-600 uppercase">
                  Filter Standard
                </label>
            ...
            {/* SMART BALANCE DISPLAY */}
            {selectedStudentLedger && !editingId && (
              <div className="mb-4 p-3 bg-white border border-green-200 rounded-lg flex flex-wrap justify-between items-center shadow-sm gap-2">
                <span className="text-sm font-bold text-gray-600">
                  Total Dues: <span className="text-gray-900">₹{selectedStudentLedger.stdFee}</span>
                </span>
                <span className="text-sm font-bold text-red-600 bg-red-50 px-3 py-1 rounded">
                  Remaining: ₹{selectedStudentLedger.remaining}
                </span>
      ...
      {viewMode === "history" && (
        <div className="w-full">
          {/* ✅ Responsive Ledger Filters */}
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <select
              value={filterStd}
              onChange={(e) => setFilterStd(e.target.value)}
              className="w-full sm:w-auto p-2 border rounded-lg outline-none bg-gray-50 font-bold"
            >
              <option value="">Filter: All Standards</option>
              {STANDARD_OPTIONS.map((std) => (
          ...
          <div className="w-full overflow-x-auto border border-gray-100 rounded-xl shadow-sm">
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="bg-gray-50 border-b">
                  <th className="p-4 text-sm text-gray-600">Student Info</th>
                  <th className="p-4 text-sm text-gray-600 text-center">Total Fee</th>
                  <th className="p-4 text-sm text-gray-600 text-center">Paid</th>
                  <th className="p-4 text-sm text-gray-600 text-center">Remaining</th>
                  <th className="p-4 text-sm text-gray-600 text-center">Statement</th>
                </tr>
              </thead>
              <tbody>
                {filteredLedger.length === 0 ? (
                  ...
                  filteredLedger.map((student) => (
                    <React.Fragment key={student._id}>
                      <tr className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-4 text-center font-bold text-green-600 text-sm">
                          ₹{student.totalPaid.toLocaleString()}
                        </td>
                        <td className="p-4 text-center font-bold text-red-600 text-sm">
                          ₹{student.remaining.toLocaleString()}
                        </td>
                        <td className="p-4 flex justify-center gap-2">
                          <button
                            ...
                            <div className="bg-white border rounded-lg p-4">
                              ...
                              {student.payments.length === 0 ? (
                                ...
                                student.payments.map((p) => (
                                  <div
                                    key={p._id}
                                    className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-3 border-b border-gray-50 last:border-0 gap-2"
                                  >
                                    ...
                                    <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                                      ...
                                      <div className="flex gap-2">
                                        <button
                                          onClick={() => handleEditClick(p)}
                                          className="text-orange-500 hover:text-orange-700 p-1 bg-orange-50 rounded"
                                        >
                                          <Edit size={14} />
                                        </button>
                                        <button
      ...
      {viewMode === "structure" && (
        <div className="max-w-2xl w-full">
          ...
          <form
            onSubmit={handleSaveStructure}
            className="flex flex-col sm:flex-row gap-4 items-end mb-8 bg-gray-50 p-4 rounded-xl border border-gray-200"
          >
            ...
            <button
              type="submit"
              className="w-full sm:w-auto bg-gray-900 text-white font-bold py-2 px-6 rounded shadow hover:bg-gray-800"
            >
              Save Setup
            </button>
          </form>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {structures.map((s) => (
              <div
                key={s._id}
                className="border border-gray-200 p-4 rounded-lg shadow-sm text-center bg-white"
              >
                <p className="text-xs font-bold text-gray-500 uppercase">
                  {s.std}
                </p>
