...
import { IndianRupee, PlusCircle, Settings, History, Trash2, Edit, Download, ChevronDown, ChevronUp } from 'lucide-react';
...
const FeesTab = () => {
  ...
  const ledger = students.map(student => {
    ...
    const studentPayments = payments.filter(p => p.studentId?._id === student._id);
  ...
  const filteredLedger = ledger.filter(s => (!filterStd || s.std === filterStd) && (!filterBatch || s.batch === filterBatch));

  // Selected Student Details for Collection Validation
  const selectedStudentLedger = ledger.find(s => s._id === formData.studentId);

  // --- SETUP TAB FIX ---
  const handleSaveStructure = async (e) => {
    e.preventDefault();
    if (!structData.std || !structData.totalFee) return alert("Please fill all fields");
    try {
  ...
  const handleCollectPayment = async (e) => {
    ...
    if (!editingId && selectedStudentLedger && Number(formData.amountPaid) > selectedStudentLedger.remaining) {
      return alert(`Cannot accept ₹${formData.amountPaid}. The student only owes ₹${selectedStudentLedger.remaining}.`);
    }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 border-b border-gray-100 pb-4 gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto bg-gray-100 p-1 rounded-lg">
          ...
          <button onClick={() => setViewMode('history')} className={`flex-1 md:flex-none px-4 py-2 text-sm font-bold rounded-md ${viewMode === 'history' ? 'bg-white shadow-sm text-green-700' : 'text-gray-500 hover:text-gray-800'}`}>Ledger</button>
      ...
      {viewMode === 'collect' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <form onSubmit={handleCollectPayment} className="bg-green-50/50 border border-green-100 p-6 rounded-xl relative">
            <h3 className="font-bold text-green-900 mb-4 flex items-center gap-2"><PlusCircle size={18}/> {editingId ? 'Edit Payment' : 'New Receipt'}</h3>
            {editingId && <button type="button" onClick={() => { setEditingId(null); setFormData({ studentId: '', amountPaid: '', paidBy: '', paymentMode: 'Cash' }); }} className="absolute top-6 right-6 text-sm text-red-600 font-bold">Cancel Edit</button>}
            ...
            <div className="mb-4">
              ...
              <select required disabled={editingId} value={formData.studentId} onChange={e => setFormData({...formData, studentId: e.target.value})} className="w-full p-2 border rounded mt-1 bg-white font-medium">
                ...
                {filteredLedger.map(s => <option key={s._id} value={s._id}>{s.name} ({s.std})</option>)}
              </select>
            </div>
            ...
            {selectedStudentLedger && !editingId && (
              <div className="mb-4 p-3 bg-white border border-green-200 rounded-lg flex justify-between items-center shadow-sm">
                <span className="text-sm font-bold text-gray-600">Total Dues: <span className="text-gray-900">₹{selectedStudentLedger.stdFee}</span></span>
                <span className="text-sm font-bold text-red-600 bg-red-50 px-3 py-1 rounded">Remaining: ₹{selectedStudentLedger.remaining}</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input required type="number" min="1" max={!editingId && selectedStudentLedger ? selectedStudentLedger.remaining : undefined} value={formData.amountPaid} onChange={e => setFormData({...formData, amountPaid: e.target.value})} className="w-full p-2 border rounded mt-1 font-bold text-green-700" />
              </div>
            ...
            <button type="submit" disabled={!editingId && selectedStudentLedger?.remaining === 0} className="w-full bg-green-600 text-white font-bold py-3 rounded-lg shadow-md hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed">
              {!editingId && selectedStudentLedger?.remaining === 0 ? 'Fee Fully Paid!' : editingId ? 'Update Receipt' : 'Generate Receipt'}
            </button>
          </form>
        </div>
      )}
      ...
      {viewMode === 'history' && (
        <div>
          <div className="flex gap-4 mb-4">
            <select value={filterStd} onChange={e => setFilterStd(e.target.value)} className="p-2 border rounded-lg outline-none bg-gray-50 font-bold"><option value="">Filter: All Standards</option>{STANDARD_OPTIONS.map(std => <option key={std} value={std}>{std}</option>)}</select>
            <select value={filterBatch} onChange={e => setFilterBatch(e.target.value)} className="p-2 border rounded-lg outline-none bg-gray-50 font-bold"><option value="">Filter: All Batches</option><option value="Morning">Morning</option><option value="Evening">Evening</option></select>
          </div>

          <div className="overflow-x-auto border border-gray-100 rounded-xl shadow-sm">
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead><tr className="bg-gray-50 border-b"><th className="p-4 text-sm text-gray-600">Student Info</th><th className="p-4 text-sm text-gray-600 text-center">Total Fee</th><th className="p-4 text-sm text-gray-600 text-center">Paid</th><th className="p-4 text-sm text-gray-600 text-center">Remaining</th><th className="p-4 text-sm text-gray-600 text-center">Statement</th></tr></thead>
              <tbody>
                {filteredLedger.length === 0 ? <tr><td colSpan="5" className="p-8 text-center text-gray-500">No students found.</td></tr> :
                  filteredLedger.map(student => (
                    <React.Fragment key={student._id}>
                      <tr className="border-b hover:bg-gray-50">
                        <td className="p-4">
                          <p className="font-bold text-gray-900">{student.name}</p>
                          <p className="text-xs text-gray-500">{student.std} • {student.batch}</p>
                        </td>
                        <td className="p-4 text-center font-bold text-gray-600">₹{student.stdFee.toLocaleString()}</td>
                        <td className="p-4 text-center font-bold text-green-600">₹{student.totalPaid.toLocaleString()}</td>
                        <td className="p-4 text-center font-bold text-red-600">₹{student.remaining.toLocaleString()}</td>
                        <td className="p-4 flex justify-center gap-2">
                          <button onClick={() => handleDownloadStatement(student)} className="px-3 py-1.5 bg-blue-50 text-blue-700 font-bold rounded hover:bg-blue-100 text-xs flex items-center gap-1" title="Download Statement"><Download size={14} /> PDF</button>
                          <button onClick={() => setExpandedStudentId(expandedStudentId === student._id ? null : student._id)} className="px-2 py-1.5 bg-gray-100 text-gray-700 rounded hover:bg-gray-200">
                            {expandedStudentId === student._id ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
                          </button>
                        </td>
                      </tr>
                        ...
                        <tr className="bg-gray-50">
                          <td colSpan="5" className="p-4 border-b">
                            <div className="bg-white border rounded-lg p-4">
                              <h4 className="text-sm font-bold text-gray-700 mb-3 border-b pb-2">Individual Receipts</h4>
                              {student.payments.length === 0 ? <p className="text-xs text-gray-500">No payments yet.</p> : 
                                student.payments.map(p => (
                                  <div key={p._id} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                                    <div className="text-sm"><span className="font-bold text-gray-800">{new Date(p.date).toLocaleDateString()}</span> - {p.paymentMode} (By {p.paidBy})</div>
                                    <div className="flex items-center gap-4">
                                      <span className="font-bold text-green-600">₹{p.amountPaid}</span>
                                      <button onClick={() => handleEditClick(p)} className="text-orange-500 hover:text-orange-700"><Edit size={14}/></button>
                                      <button onClick={() => handleDeletePayment(p._id)} className="text-red-500 hover:text-red-700"><Trash2 size={14}/></button>
                                    </div>
                                  </div>
                                ))
                              }
        ...
        <div className="max-w-2xl">
          ...
          <form onSubmit={handleSaveStructure} className="flex gap-4 items-end mb-8 bg-gray-50 p-4 rounded-xl border border-gray-200">
            <div className="flex-1">
              <label className="font-bold text-xs text-gray-600 uppercase">Standard</label>
              <select value={structData.std} onChange={e => setStructData({...structData, std: e.target.value})} className="w-full p-2 border rounded mt-1 bg-white">
            ...
            <button type="submit" className="bg-gray-900 text-white font-bold py-2 px-6 rounded shadow hover:bg-gray-800">Save Setup</button>
          </form>
