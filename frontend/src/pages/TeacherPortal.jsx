...
//       {messages.length === 0 ? (
//         <div className="p-8 text-center text-gray-500 bg-gray-50 border border-dashed rounded-xl">No new messages or announcements.</div>
//       ) : (
//         messages.map((msg) => (
//           <div key={msg._id} className="bg-purple-50/50 rounded-xl border border-purple-100 p-5 hover:shadow-md transition-shadow">
//             <div className="flex justify-between items-start mb-3 border-b border-purple-100 pb-3">
//               <div className="flex items-center gap-3">
//                 <div className="h-10 w-10 bg-purple-600 text-white rounded-full flex items-center justify-center font-bold">
//                   {msg.sender?.name ? msg.sender.name.charAt(0).toUpperCase() : 'U'}
//                 </div>
//                 <div>
//                   <p className="font-bold text-gray-900">{msg.sender?.name || 'Unknown User'}</p>
//                   <p className="text-xs font-medium text-purple-600 uppercase tracking-wider">To: {msg.recipientGroup}</p>
//                 </div>
//               </div>
...
//             <div><label className="block text-sm font-bold text-purple-900 mb-1">Select Date</label><input type="date" value={addDate} onChange={(e) => setAddDate(e.target.value)} className="w-full p-2 border rounded" /></div>
//             <div><label className="block text-sm font-bold text-purple-900 mb-1">Standard</label><select value={addStd} onChange={e => setAddStd(e.target.value)} className="w-full p-2 border rounded"><option value="All">All Standards</option>{standardOptions.map(std => <option key={std} value={std}>{std}</option>)}</select></div>
//             <div><label className="block text-sm font-bold text-purple-900 mb-1">Batch</label><select value={addBatch} onChange={e => setAddBatch(e.target.value)} className="w-full p-2 border rounded"><option value="All">All Batches</option><option value="Morning">Morning</option><option value="Evening">Evening</option></select></div>
//           </div>
...
//             <div><label className="block text-sm font-bold text-gray-700 mb-1">Date</label><input type="date" value={fetchDate} onChange={(e) => setFetchDate(e.target.value)} className="w-full p-2 border rounded" /></div>
...
//               <thead><tr className="bg-gray-100 border-b"><th className="p-3">Student Name</th><th className="p-3">Standard</th><th className="p-3">Batch</th><th className="p-3">Status</th></tr></thead>
//               <tbody>
//                 {displayedRecords.length === 0 ? (<tr><td colSpan="4" className="p-8 text-center text-gray-500">No records to display.</td></tr>) : (
//                   displayedRecords.map((record, idx) => (
//                     <tr key={idx} className="border-b hover:bg-gray-50"><td className="p-3 font-bold text-gray-900">{record.name}</td><td className="p-3 text-sm text-gray-600">{record.std}</td><td className="p-3 text-sm text-gray-600">{record.batch}</td><td className="p-3"><span className={`px-3 py-1 rounded-full text-xs font-bold ${record.status === 'Present' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{record.status}</span></td></tr>
//                   ))
//                 )}
//               </tbody>
//             </table>
...
//   return (
//     <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
//       <h2 className="text-xl font-bold mb-6 flex items-center gap-2"><Send className="text-purple-600" /> Post an Update</h2>
//       <form onSubmit={handleSend}>
//         <select value={recipient} onChange={(e) => setRecipient(e.target.value)} className="w-full md:w-1/2 p-3 border border-gray-200 rounded-lg mb-4">
//           <option value="All Students">To: All Students</option>
//           <option value="All Staff & Admin">To: All Staff & Admin</option>
//         </select>
//         <textarea value={newContent} onChange={e => setNewContent(e.target.value)} rows="4" className="w-full p-3 border rounded-lg mb-4" placeholder="Type your announcement here..." required></textarea>
//         <button type="submit" className="bg-purple-600 text-white px-6 py-3 rounded-lg font-bold">Broadcast Message</button>
//       </form>
//     </div>
//   );
// };
...
import { 
  BookOpen, LayoutDashboard, CheckSquare, MessageSquare, 
  Send, Bell, Clock, GraduationCap, FileText, Menu, X 
} from 'lucide-react';
