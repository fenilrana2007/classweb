  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6">
      ...
      {tabMode === 'record' && (
        <div className="animate-fade-in">
          {/* ✅ Responsive Inputs */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 mb-6 bg-purple-50 p-4 rounded-xl border border-purple-100">
            <div><label className="block text-xs md:text-sm font-bold text-purple-900 mb-1">Select Date</label><input type="date" value={addDate} onChange={(e) => setAddDate(e.target.value)} className="w-full p-2.5 border rounded-lg text-sm" /></div>
            <div><label className="block text-xs md:text-sm font-bold text-purple-900 mb-1">Standard</label><select value={addStd} onChange={e => setAddStd(e.target.value)} className="w-full p-2.5 border rounded-lg text-sm bg-white"><option value="All">All Standards</option>{standardOptions.map(std => <option key={std} value={std}>{std}</option>)}</select></div>
            <div><label className="block text-xs md:text-sm font-bold text-purple-900 mb-1">Batch</label><select value={addBatch} onChange={e => setAddBatch(e.target.value)} className="w-full p-2.5 border rounded-lg text-sm bg-white"><option value="All">All Batches</option><option value="Morning">Morning</option><option value="Evening">Evening</option></select></div>
          </div>
          ...
          <button onClick={handleSubmitAttendance} className="w-full bg-purple-600 text-white py-4 md:py-3 rounded-lg font-bold hover:bg-purple-700 transition-colors shadow-md">Save Attendance for {addDate}</button>
        </div>
      )}
        ...
        <div className="animate-fade-in">
           ...
           <div className="flex flex-col lg:flex-row gap-3 md:gap-4 mb-6 bg-gray-50 p-4 rounded-xl border border-gray-200 lg:items-end">
            ...
            <div className="flex flex-wrap sm:flex-nowrap gap-2 w-full lg:w-auto h-[42px] mt-2 lg:mt-0">
              <button onClick={() => handleFetchAttendance()} disabled={isFetching} className="flex-1 sm:flex-none px-4 bg-gray-900 text-white rounded-lg font-bold hover:bg-gray-800 text-sm shadow-md h-full">{isFetching ? 'Loading...' : 'Fetch'}</button>
              <button onClick={handleExportCurrentView} className="flex-1 sm:flex-none px-4 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold text-sm shadow-sm flex items-center justify-center h-full"><Download size={18}/></button>
...
const MessagesTab = ({ messages, setMessages, user }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 animate-fade-in max-w-4xl">
      <h2 className="text-lg md:text-xl font-bold mb-4 md:mb-6 flex items-center gap-2"><Send className="text-purple-600" /> Post an Update</h2>
      <form onSubmit={handleSend}>
        <select value={recipient} onChange={(e) => setRecipient(e.target.value)} className="w-full sm:w-1/2 p-3 md:p-3 border border-gray-200 rounded-lg mb-4 text-sm md:text-base bg-white">
          <option value="All Students">To: All Students</option>
          <option value="All Staff & Admin">To: All Staff & Admin</option>
        </select>
        <textarea value={newContent} onChange={e => setNewContent(e.target.value)} rows="5" className="w-full p-3 md:p-3 border border-gray-200 rounded-lg mb-4 text-sm md:text-base resize-y outline-none focus:ring-2 focus:ring-purple-600" placeholder="Type your announcement here..." required></textarea>
        <button type="submit" className="w-full sm:w-auto bg-purple-600 text-white px-8 py-3 rounded-lg font-bold hover:bg-purple-700 shadow-md transition-colors">Broadcast Message</button>
      </form>
    </div>
  );
};
